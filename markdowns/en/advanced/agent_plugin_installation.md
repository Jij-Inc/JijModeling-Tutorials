...
The `jijmodeling` plugin uses the [Agent Plugins 1.0](https://agent-plugins.org/) manifest (`plugin.json`) and includes Claude Code metadata for marketplace installation and updates. Claude Code and Cursor can load this plugin directly. Codex, GitHub Copilot, and VS Code use the bundled skill through the installation methods below.

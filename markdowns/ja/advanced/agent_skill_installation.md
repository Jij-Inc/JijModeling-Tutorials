...
また、同梱のスキルは機能追加などに応じて更新されていきますので、利用する JijModeling のバージョンを変更した場合は、以下で説明するインストール方法を再度繰り返し、必要に応じてスキルを更新することをお勧めします。

:::{admonition} スキルは万能ではない
:class: caution

スキルは、コーディングエージェントによる JijModeling の利用を補助しますが、**結果的に生成されるコードや定式化が適切であることを保証するものではありません**。
スキルの役割は、あくまでもコーディングエージェントが JijModeling をスムーズに書けるようにすることである点に注意しましょう。
:::

## スキルのインストール

以下では、JijModeling に同梱されているスキルのインストール方法について説明します。

### スキルのパスを確認する

スキルをインストールするには、スキルがどこに配置されているのかを知る必要があります。
`uv` を使って管理されているプロジェクトでは、JijModeling に同梱されているスキルのパスを以下のコマンドにより取得できます：
...
`jijmodeling skill path` コマンドで出力されたディレクトリ以下のファイルを、お使いのエージェントのマニュアルの指示に従ってコピーすれば、エージェントの次回起動時からスキルが自動で使われるようになります。

また、**スキルマネージャー**を使うと、プロジェクト単位か個人単位なのか、またどのエージェントで利用できるようにするのか、といった情報を与えることで自動的にインストールしてくれるようになり便利です。
スキルマネージャーにはいくつか既存実装がありますが、以下では一般的に用いられている GitHub CLI でのインストール方法を紹介します。
...
以下の作業は、上記のリンクから十分新しい GitHub CLI（`gh` コマンド）をインストールした上で行うようにしてください。

...
これに対処するため、JijModeling 2.7.0 以降では、{py:meth}`~jijmodeling.Problem.update` 関数の第 2 引数以降で {py:class}`~jijmodeling.Problem` 内で定義済みの変数（プレースホルダー、カテゴリーラベル、決定変数、名前付き数式）を引数として束縛できるようになっています。
これらの要素は Problem から引数名に基づいて自動的に取得されるため、`update` 内で必要な変数を引数から取得できるようになります。
追加引数の指定方法については「{ref}`update_parameters`」を参照してください。

2.7.0 以前をお使いの場合は、次節以降で説明する方法で `Problem` が持つメタデータを改めて引き出してから更新処理を行うことで、同等の処理が可能です。
:::

...
`sense`は数理モデルが最大化問題（{py:attr}`jm.ProblemSense.MAXIMIZE <jijmodeling.ProblemSense.MAXIMIZE>`）と最小化問題（{py:attr}`jm.ProblemSense.MINIMIZE <jijmodeling.ProblemSense.MINIMIZE>`）のどちらであるかを指定する引数であり、省略した場合最小化問題として扱われます。
...
それでは、次章から具体的に問題の構築に必要な機能の各論に入っていきましょう。

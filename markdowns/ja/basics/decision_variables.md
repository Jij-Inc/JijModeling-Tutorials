...
以下では、数理モデルの重要な構成要素の一つである**決定変数**について、JijModeling における概念的な説明と宣言方法を見ていきます。
...
また、以下で例として用いるため、前節までに定義してきたナップサック問題の定式化を再掲しておきます。
...
TODO: 基本的には、決定変数から `*Var` を取ったものがプレースホルダーとしてだと思っておけばよいですが、`Float` のみ名前が違うことに留意してください。

...
3. **{doc}`placeholders`**：数理最適化問題のコンパイルに必要な入力データである**プレースホルダー**の宣言方法と、式中での役割について説明します。
4. **{doc}`decision_variables`**：数理最適化問題を構成する**決定変数**との宣言方法と、その式中での役割を解説します。
5. **{doc}`arithmetics_and_comparison`**：式の中で使える算術演算や比較演算について説明します。
6. **{doc}`arrays_and_dicts`**：式の中で使える配列や辞書などのデータ構造について説明します。
7. **{doc}`set_and_logical_ops`**：配列や辞書を集合として総和をとったり、論理演算により組み合わせる方法を説明します。。
8. **{doc}`modeling`**：これまでの要素を用いて、数理モデルの目的関数や制約条件を設定する方法を紹介します。
9. **{doc}`instance_generation`**：インタンスデータを入力しインスタンスを生成する方法を学びます。また、制約検出の制御についても簡単に触れます。

...
逆に言えば、複数のインスタンス間で決定変数や制約条件の ID の対応関係を共有したい場合、個別に {py:meth}`jijmodeling.Problem.eval` メソッドを呼び出すのではなく、単一の {py:class}`~jijmodeling.Compiler` オブジェクトを共有してコンパイルする必要があります。

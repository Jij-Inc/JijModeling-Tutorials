...
このような変数の族は、総和記号などを含む一般の数理モデルの定式化の際に必要になります。
...
また、タプルの配列によりグラフ構造を表したり、タプルをキーとする辞書により辺にラベルのついたグラフ構造を表すこともできます。
...
以下、配列と辞書のそれぞれについて簡単に概要を述べます。より一般の事項については {doc}`./expressions` を参照してください。
